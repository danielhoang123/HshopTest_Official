/* MMA8452Q Example Code
   ...
   Arduino Mega boards have SDA on pin 20, and SCL on pin 21.  Please wire your board accordingly.
...
void loop()
{
  ...
  */
    ...
    float fact = (1<<12)/(2*SCALE);
    float mag = 0;
    for (int i=0; i<3; i++){
      accelG[i] = (float) accelCount[i]/fact;  // get actual g value, this depends on scale being set
      mag = mag + accelG[i] * accelG[i];
    }
    ...
    {
      if (accelG[i] > 1.02){
        Serial.println(accelG[i], 2);  // Print g values
      }
