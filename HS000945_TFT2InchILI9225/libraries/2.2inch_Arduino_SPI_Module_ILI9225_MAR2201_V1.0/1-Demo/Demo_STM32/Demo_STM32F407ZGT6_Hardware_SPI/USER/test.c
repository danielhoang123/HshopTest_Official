...
//本模块默认数据总线类型为SPI总线
//     LCD模块                STM32单片机    
//       SDI         接          PB5          //液晶屏SPI总线数据写信号
//=======================================液晶屏控制线接线==========================================//
//     LCD模块 					      STM32单片机 
//       LED         接          PB13         //液晶屏背光控制信号，如果不需要控制，接5V或3.3V
//       CLK         接          PB3          //液晶屏SPI总线时钟信号
//       RS          接          PB14         //液晶屏数据/命令控制信号
...
void main_test(void)
{
	...
	Gui_StrCenter(0,55,RED,BLUE,"综合测试程序",16,1);//居中显示	
	Gui_StrCenter(0,80,GREEN,BLUE,"2.2\" ILI9225 176X220",16,1);//居中显示
	Gui_StrCenter(0,105,BLUE,BLUE,"xiaoFeng@QDtech",16,1);//居中显示
	Gui_StrCenter(0,130,BLUE,BLUE,"2018-12-04",16,1);//居中显示
	delay_ms(1500);		
	delay_ms(1500);
}
...
void Test_FillRec(void)
{
	...
	for (i=0; i<5; i++) 
	{
		...
		LCD_DrawRectangle(lcddev.width/2-65+(i*15),lcddev.height/2-60+(i*15),lcddev.width/2-65+(i*15)+60,lcddev.height/2-60+(i*15)+60); 
	}
	delay_ms(1500);	
	LCD_Fill(0,20,lcddev.width-1,lcddev.height-20,WHITE); 
	for (i=0; i<5; i++) 
	{
		POINT_COLOR=ColorTab[i];
		LCD_DrawFillRectangle(lcddev.width/2-65+(i*15),lcddev.height/2-60+(i*15),lcddev.width/2-65+(i*15)+60,lcddev.height/2-60+(i*15)+60); 
	}
	delay_ms(1500);
}
...
void Test_Circle(void)
{
	...
	LCD_Fill(0,20,lcddev.width-1,lcddev.height-20,WHITE);
	for (i=0; i<5; i++)  
		gui_circle(lcddev.width/2-55+(i*25),lcddev.height/2-30+(i*15),ColorTab[i],30,0);
	delay_ms(1500);	
	LCD_Fill(0,20,lcddev.width-1,lcddev.height-20,WHITE); 
	for (i=0; i<5; i++) 
	  	gui_circle(lcddev.width/2-55+(i*25),lcddev.height/2-30+(i*15),ColorTab[i],30,1);
	delay_ms(1500);
}
...
{
	u8 i=0;
	DrawTestPage("测试4:GUI Tri填充测试");
	LCD_Fill(0,20,lcddev.width-1,lcddev.height-20,WHITE);
	for(i=0;i<5;i++)
	{
		POINT_COLOR=ColorTab[i];
		Draw_Triangel(lcddev.width/2-80+(i*20),lcddev.height/2-5+(i*15),lcddev.width/2-50-1+(i*20),lcddev.height/2-5-52-1+(i*15),lcddev.width/2-20-1+(i*20),lcddev.height/2-5+(i*15));
	}
	delay_ms(1500);	
	LCD_Fill(0,20,lcddev.width-1,lcddev.height-20,WHITE); 
	for(i=0;i<5;i++)
	{
		POINT_COLOR=ColorTab[i];
		Fill_Triangel(lcddev.width/2-80+(i*20),lcddev.height/2-5+(i*15),lcddev.width/2-50-1+(i*20),lcddev.height/2-5-52-1+(i*15),lcddev.width/2-20-1+(i*20),lcddev.height/2-5+(i*15));
	}
	delay_ms(1500);
}
...
void Pic_test(void)
{
	...
	//LCD_Fill(0,20,lcddev.width,lcddev.height-20,WHITE);
	Gui_Drawbmp16(10,30,gImage_qq);
	Show_Str(10+12,75,BLUE,YELLOW,"QQ",16,1);
	Gui_Drawbmp16(70,30,gImage_qq);
	Show_Str(70+12,75,BLUE,YELLOW,"QQ",16,1);
	Gui_Drawbmp16(130,30,gImage_qq);
	Show_Str(130+12,75,BLUE,YELLOW,"QQ",16,1);
	delay_ms(1200);
}
